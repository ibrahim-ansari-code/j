...
- **Time on page**: Total time on page is sent to `POST /beacon-time` when the user hides the tab or leaves.
- **Section visibility**: Elements with `data-landright-section` (e.g. Hero, Features, Footer) are observed; when a section leaves view or the user leaves the page, view duration is sent to `POST /beacon-time` with `section_id` and to PostHog as `section_viewed` events.
